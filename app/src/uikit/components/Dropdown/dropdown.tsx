...
const Dropdown: ParentComponent<DropdownProps<any>> = (props) => {
  ...
  createEffect(() => {
    if (isOpen() && itemsWrapperRef && inputContainerRef) {
  ...
  const DropdownItems: ParentComponent<{
    ...
    return <div class={css["items"]}>{filtered()}</div>;
  };
  ...
  createEffect(() => {
    ...
    if (value) {
      animate(
        chevron,
        av.elementsPoints.dropdownChevron.open,
        av.defaultAnimationType,
      );
    } else {
      animate(
        chevron,
        av.elementsPoints.dropdownChevron.close,
        av.defaultAnimationType,
      );
    }
  });
    ...
    <DropdownContext.Provider value={contextValue}>
      <div class={css["wrapper"]}>
        <div class={cssInput["wrapper"]}>
          ...
          <Show when={props.actionChildren}>{props.actionChildren}</Show>
        </div>
        <Show when={isOpen()}>
          <div class={css["dropdown-area"]} ref={itemsWrapperRef}>
            <DropdownItems search={search}>{props.children}</DropdownItems>
          </div>
        </Show>
      </div>
