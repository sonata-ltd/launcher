...
export const ContentStack: Component<ContentStackProps> = (props) => {
	...
	onMount(() => {
		requestAnimationFrame(() => {
			const e = childRefs[props.index()]; // ← реальный DOM-элемент из <For>
			if (!e) return;

			const eClientRects = e.getBoundingClientRect();

			if (eClientRects) {
				if (!containerRef) return;
				(
					containerRef as HTMLDivElement
				).style.height = `${eClientRects.height}px`;
			} else {
				console.warn("client rects not found");
			}
		});
	});
			...
			if (containerRef) {
				...
				animate(
					...
					eClientReacts.width * props.index(),
					{
						...
						onUpdate: (v) =>
							((containerRef as HTMLElement).scrollLeft = v),
					},
				);
