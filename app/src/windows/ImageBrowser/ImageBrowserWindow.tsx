...
import {
	InsertionImagesStore,
	LocalImageElement,
} from "lib/dbInterface/images/handler";
...
import {
	...
	WindowControls,
	WindowFrame,
} from "uikit/components/Window/window";
...
import { ImageBrowserResult } from "./types";

export const ImageBrowserWindow: Component<
	WindowComponentProps<null, ImageBrowserResult>
> = (props) => {
	const [selectedImage, setSelectedImage] = createSignal<
		undefined | { name: string; secondAttribute: string; preview: string }
	>(undefined);

	const [insertionImages, setInsertionImages] =
		createStore<InsertionImagesStore>({
			images: [],
			inserted: -1,
			lastOperation: {
				isRunning: false,
	...
	const [lastInsertion, setLastInsertion] = createStore(
		insertionImages.lastOperation,
	);

	const [dbMethods] = useDBData();

	const [infoContainer, setInfoContainer] = createSignal<HTMLDivElement>();
	const [uploadingContainer, setUploadingContainer] =
		createSignal<HTMLDivElement>();
	const [uploadingContentWrapper, setUploadingContentWrapper] =
		createSignal<HTMLDivElement>();
	const [uploadingMessage, setUploadingMessage] =
		createSignal<string>("Uploading 0/0");
	...
	const animateUploadingContainer = (show: boolean, fn?: () => void) => {
		...
		const totalWidth =
			spinnerWidth + uploadingMessagePadding + digitsPadding;
	...
	createEffect(() => {
		setUploadingMessage(
			`Uploading ${lastInsertion.inserted}/${lastInsertion.total}`,
		);
	});
	...
	const selectImage = (
		...
		if (preview !== undefined) {
			setSelectedImage({ name, secondAttribute, preview });
			// props.setImageSrc(preview);
		}
	};

	const handleClose = () => {
		const selectedImageObject = selectedImage();
		if (selectedImageObject !== undefined) {
			props.close({ imageSrc: selectedImageObject.preview });
		} else {
			props.close();
		}
	};
					...
					<Show
						...
						fallback={
							<div class={css["indicator-wrapper"]}>
								<ContentLoadingIndicator processName="Extracting Images..." />
							</div>
						}
					>
						<div class={css["image-browser"]}>
							<div class={css["controls-wrapper"]}>
								<Button primary onClick={() => pickImage()}>
									Add
								</Button>
								<div
