...
import {
	restoreWindow,
	useMinimizedCount,
	useMinimizedWindowsIds,
} from "lib/windowManager/store";
...
export const Tasklist = (props: TasklinkProps) => {
	...
	createEffect(() => {
		props.wsMessagesTracked()?.forEach((msg) => {
			const validatedTask = validateMessageTypeCustom(
				taskSchema,
				msg.task,
			);
			...
			if (validatedTask) {
				const isActive = () =>
					validatedTask.data.status === "Pending" ||
					validatedTask.data.status === "Running";

				const exists = registeredTasks.some(
					(t) => t.self.id === validatedTask.id,
				);
				if (!exists) {
					...
					const idx = registeredTasks.findIndex(
						(t) => t.self.id === validatedTask.id,
					);
	...
	const [initTasklistScrollbars, getTasklistScrollbarsInstance] =
	...
	const minimizedWindowsIds = useMinimizedWindowsIds();
	const minimizedWindowsCount = useMinimizedCount();
		...
		<>
			...
			>
				...
				<div class={css["details-container"]}>
					...
					>
						<p class={css["details"]}>{statusText()}</p>
					</Show>
				</div>
			</div>
		</>
