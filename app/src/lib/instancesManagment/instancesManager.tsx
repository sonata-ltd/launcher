...
type Store = [
	{
		...
		getInstanceById: (id: number) => Accessor<InstanceInfo>;
		runInstance: (instance: InstanceInfo) => void;
		getVersionUrl: (
			versionId: string,
		) => UnifiedVersionInfo["url"] | undefined;
...
export function InstancesStateProvider(props: { children: JSX.Element }) {
	...
	let manifestVersionsMap = new Map<
		UnifiedVersionInfo["id"],
		UnifiedVersionInfo["url"]
	>();
	...
	const getInstanceList = createMemo(() => Object.values(instances));
	const getInstanceById = (id: number) => createMemo(() => instances[id]);
...
export function useInstancesState() {
	...
	if (!context) {
		throw new Error(
			"useInstancesState must be used inside InstancesStateProvider",
		);
	}
